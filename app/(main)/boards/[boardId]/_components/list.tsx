    ...
    <Droppable droppableId={`list-${id}`}>
      {(provided, snapshot) => (
        <div
          ...
          className={cn(
            snapshot.isDraggingOver && "bg-blue-600",
            "rounded-md p-3 bg-muted text-muted-foreground w-[310px] flex-shrink-0"
          )}
        ...
        >
          ...
          <div className="flex flex-col gap-3 my-4">
            ...
            {cards.length > 0 &&
              cards.map((card, i) => (
                <Draggable key={card.id} draggableId={`card-${card.id}`} index={i}>
                  {(provided, snapshot) => (
                    <div
                      ref={provided.innerRef}
